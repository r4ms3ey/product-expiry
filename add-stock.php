...
if (isset($_POST["btnsave"])) {
  ...
  $productID = trim($_POST['txtproductID'] ?? '');
  $stockDate = trim($_POST['txtstockinDate'] ?? '');
  $drugName = trim($_POST['cmddrug'] ?? '');
  $unitPrice = trim($_POST['txtprice'] ?? '');
  $quantity = trim($_POST['txtqty'] ?? '');
  ...
  if (
    ...
    $_SESSION['error'] = 'Stock quantity must be a non-negative number.';
  } elseif ($quantity == 0) {
    $_SESSION['error'] = 'Stock quantity cannot be zero.';
  } elseif (!is_numeric($unitPrice) || $unitPrice < 0) {
    $_SESSION['error'] = 'Unit price must be a non-negative number.';
  } else {
    ...
    if ($result) {
      ...
      $currentStock = floatval($_POST['txtstock']);
      $newQty = $currentStock + floatval($quantity);
      $sql = "UPDATE tblproduct SET qty=?, price=? WHERE product_name=?";
      $stmt = $dbh->prepare($sql);
      $stmt->execute([$newQty, $unitPrice, $drugName]);
