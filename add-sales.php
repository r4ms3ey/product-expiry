...
if(isset($_POST["btnsave"])){
  ...
  if ($emptyField) {
    $_SESSION['error'] = 'One or more required fields are empty.';
  } else {
    // Fetch product details for validation
    $productName = $_POST['cmddrug'];
    $sql = "SELECT qty, expirydate FROM tblproduct WHERE product_name = ?";
    ...
    if(!$product){
      ...
      $currentStock = (int)$product['qty'];
      $requestedQty = (int)$_POST['txtqty'];
      $expiryDate = $product['expirydate'];

      // Check for expiry
      if(strtotime($expiryDate) < strtotime(date('Y-m-d'))){
        $_SESSION['error'] = 'Cannot sell expired product!';
      } else if($currentStock < 0) {
        $_SESSION['error'] = 'order out of stock we are sorry!';
      } else if($currentStock===0) {
        $_SESSION['error'] = 'order out of stock we are sorry!';
      } else if($requestedQty < 0) {
        $_SESSION['error'] = 'Invalid quantity requested!';
        ...
        if ($statement){
          // Update stock
          $newQty = $currentStock - $requestedQty;
          $sql = "UPDATE tblproduct SET qty=? WHERE product_name=?";
          $stmt= $dbh->prepare($sql);
          $stmt->execute([$newQty, $productName]);
